...
export function pipe(...fns) {
  ...
  return function (inp) {
    return fns.reduce((prev, next) => next(prev), inp)
  }
}
...
export function compose(...fns) {
  ...
  return function (inp) {
    return fns.reduceRight((prev, next) => next(prev), inp)
  }
...
export function createRecipe(steps) {
  ...
  for (let i = 0; i < steps.length; i++) {
    let elem = steps[i]
    if (elem == 'grind') obj[elem] = grind;
    else if (elem == 'roast') obj[elem] = roast;
    else if (elem == 'pack') obj[elem] = pack;
    else if (elem == 'mix') obj[elem] = mix;
    else continue;
  }
